...
decrypt_tls_response = tlsn.decrypt_tls_responseV4
eq = utils.eq
...
//override
import_resource = async function (_path) {
...
async function main (argv) {
    ...
    if (argv[0] === 'notarize') {
        ...
        // }
        var server = argv[1]
        var headersfile = argv[2]
        var headers = headersfile
    ...
    if (argv[0] === 'verify') {
        ...
        // }
        var pgsgfile = argv[1]
        var pgsgBuf = fs.readFileSync(path.join(__dirname, pgsgfile))
        ...
        var cleartext = rv[0]
        var d = new Date();
        var creationTime = d.getTime();
        var dirname = await createNewSession(creationTime, server_name, 'notary name', cleartext, pgsg, true)
        console.log('session saved in', dirname)
        return dirname
    }
